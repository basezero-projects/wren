...
fn allowed_fields_count_matches_schema_field_count() {
    ...
    // + search(10) + voice(5: enabled, model, tts_enabled, tts_voice, tts_rate)
    // + mcp(1: servers_json) = 23 tunable fields. The active model slug lives in
    // the SQLite app_config table via ActiveModelState, not in TOML. The
    // collapsed bar height and hide-commit delay are baked into the frontend
    // (see `WindowSection` doc) because they have no perceptible effect across
    // their usable range. If this assertion fails, the schema has drifted from
    // the allowlist and someone added a field without extending ALLOWED_FIELDS.
    assert_eq!(ALLOWED_FIELDS.len(), 23);
}
